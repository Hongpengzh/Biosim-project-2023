...
class BioSim:
    ...
    def __init__(self, island_map, ini_pop, seed,
        ...
        # arguments about logfile
        self._df_pop = pd.DataFrame(self.ini_pop[0]['pop'])     # read ini_pop animal information into a DataFrame
        self._df_log = pd.DataFrame()  # make a DataFrame as log file of the simulation
    ...
    def simulate_log(self, animalnum=0, carnnum=0):     # add every years' animal number to _df_log
        hiercolumn = pd.MultiIndex.from_arrays([[self.seed, self.seed], ['Herbivores', 'Carnivores']])
        if self._year == 0:
            df_log = pd.DataFrame(np.zeros((1,2)), columns=hiercolumn)
            df_log.iloc[0, 0] = self._df_pop[self._df_pop['species'] == 'Herbivore'].shape[0]
            df_log.iloc[0, 1] = self._df_pop[self._df_pop['species'] == 'Carnivore'].shape[0]
            df_log.index.name = 'year'
            df_log.columns.name = 'seed'
            self._df_log = df_log
        else:
            df_log = pd.DataFrame(np.zeros((1, 2)), columns=hiercolumn)
            df_log.iloc[0, 1] = carnnum
            df_log.iloc[0, 0] = animalnum
            self._df_log = pd.concat([self._df_log, df_log], ignore_index=True)
    ...
    def simulate(self, num_years):
        ...
        while self._year < self._final_years:
            ...
            self.simulate_log(len(animal_percell_year), len(carnivore_percell_year))
